...
- (void)windowDidLoad {
  ...
  [libOutline setDoubleAction:@selector(mwConnect:)];
...
- (void)doClickWaitMode:(id)sender {
  ...
  //NSLog(@"waiting");
  //{
  NSEvent *abortEvent = [NSApp nextEventMatchingMask:NSAnyEventMask & ~(NSFlagsChangedMask | NSPeriodicMask | NSCursorUpdateMask | NSSystemDefinedMask)  untilDate:[NSDate dateWithTimeIntervalSinceNow:0.2] inMode:NSEventTrackingRunLoopMode dequeue:NO];
...
- (BOOL)validateUserInterfaceItem:(id <NSValidatedUserInterfaceItem>)item {
  ...
  if (action == @selector(mwConnect:)) {
    ...
    return NO;
  } else if (action == @selector(mwWebSite:)) {
    NSEnumerator *e = [libOutline selectedRowEnumerator];
    NSNumber *index;
